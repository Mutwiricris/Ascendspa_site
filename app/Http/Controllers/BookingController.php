...
class BookingController extends Controller
{
    ...
    public function datetime()
    {
        try {
            ...
            for ($i = 0; $i < 90; $i++) {
                ...
                if ($date->dayOfWeek !== 0 && $date->gte(now()->startOfDay())) {
                    $availableDates[] = [
    ...
    public function selectDateTime(Request $request)
    {
        try {
            ...
            $selectedDateTime = $request->date . ' ' . $request->time;
            if (strtotime($selectedDateTime) <= strtotime('+1 hour')) {
                return back()->with('error', 'Please select a time that is at least 2 hours from now.');
            }
            ...
            $formattedDate = date('l, F j, Y', strtotime($request->date));
    ...
    public function confirmBooking(Request $request)
    {
        try {
            ...
            $booking = Booking::create([
                ...
                'end_time' => date('H:i', strtotime($bookingData['date'] . ' ' . $bookingData['time']) + $service->duration_minutes * 60),
